...
function initDeepAR() {
  // 만들어진 canvas에 deepAR 실행
  const deepAR = DeepAR({
    // 서버에서 받아오기
    licenseKey:
      "2df0063b6b8ef8eb754b707348e099d4c419524397ffeaae36f656112167e9816dafbe8dd2028e9c",
    canvasWidth: 400,
    canvasHeight: 300,
    canvas: deepARCanvas,
  ...
  deepAR.onVideoStarted = function () {
    ...
    videoInput.srcObject = deepARCanvas.captureStream(25);
  };
...
ws.onmessage = function (message) {
  ...
  switch (parsedMessage.id) {
    ...
    case "filter":
      ...
      if (filtereffect != "") {
        console.log(
          `add filter message : ${parsedMessage.id} ${parsedMessage.from} ${parsedMessage.effect}`
        );
    ...
    case "filterRemove":
      ...
      if (removeFilter != "") {
        console.log(
          `remove filter message : ${parsedMessage.id} ${parsedMessage.from} ${parsedMessage.effect}`
        );
    ...
    case "translate":
      console.log(
        `translate message : ${parsedMessage.id} ${parsedMessage.from} ${parsedMessage.text}`
      );
...
function stop(message) {
  ...
  axios
    .get("https://i7b308.p.ssafy.io/api/v1/penpal/single/user", {
      headers: {
        fromUserId: users[0],
        toUserId: users[1],
      },
    ...
    .then(function (response) {
      if (response.data === "empty") {
        ...
        // users[0] -> users[1]
        axios
          .post("https://i7b308.p.ssafy.io/api/v1/penpal", {
            fromUserId: users[0],
            toUserId: users[1],
          })
          .then(function (response) {
  ...
  axios
    .get("https://i7b308.p.ssafy.io:8080/api/v1/intimacy/single", {
      headers: {
        fromUserId: users[0],
        toUserId: users[1],
      },
    ...
    .then(function (response) {
      console.log(response);
      axios
        .put("https://i7b308.p.ssafy.io/api/v1/intimacy/positive/time", {
          fromUserId: users[0],
          toUserId: users[1],
    ...
    .catch(function (error) {
      console.log(error);
      axios
        .post("https://i7b308.p.ssafy.io/api/v1/intimacy", {
          fromUserId: users[0],
          toUserId: users[1],
