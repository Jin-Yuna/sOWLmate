...
- 구글 환경변수 설정까지 잘 하고 응답까지 잘 받았는데, 오디오 파일을 생성하는과정에서 오류가 발생하였다.   

- `Error: ENOENT: no such file or directory`

- 오류 아래 다음과 같은 자세한 내용이 있었다.
  
  ```bash
  errno: -4058,
    code: 'ENOENT',
    syscall: 'open',
    path: 'C:\\Users\\multicampus\\Desktop\\pj\\S07P12B308\\kurento\\out\\166010815572
  ```

- 코드를 열심히 뒤져도 code, syscall, open, path 같은 키워드가 쓰인 코드를 찾을 수 없었다. 아무리 봐도 path가 가장 의심스러웠다.

- path중에 out이라는 폴더가 현재 쿠렌토 폴더 내에 없기 때문에 그 폴더를 생성해주었더니 잘 동작하기 시작하였다.

- 응답을 받을 때 마다 out에 mp3파일이 쌓이기 때문에 파일 이름을 닉네임(중복값 없음)으로 하여 파일을 저장하면, 유저가 요청을 할 때마다 응답이 해당 파일에 덮어씌워져 파일이 무한정으로 불어난느 것을 방지할 수 있다.

- 다만 이 방법도 유저가 많아진다면 매우 비효울적일 것 같다.
